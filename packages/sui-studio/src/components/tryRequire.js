...
export const tryRequireMarkdown = async ({ category, component, file }) => {
  return requireFile({
    defaultValue: '',
    importFile: () => import(`!raw-loader!${__BASE_DIR__}/components/${category}/${component}/${file}.md`),
  })
}

export const tryRequireRawSrc = ({ category, component }) => {
  return tryRequireSrc({ category, component, requireContext: requireContextRawSrc })
}

export const tryRequireCore = async ({category, component}) => {
  const exports = tryRequireSrc({ category, component, requireContext: requireContextSrc })
